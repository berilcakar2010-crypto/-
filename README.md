...
Proje, kod deposunda **gradlew dosyası (Gradle wrapper) taşımaz** — bunun yerine hem
GitHub Actions iş akışları hem de yerel kurulum, resmi `gradle/actions/setup-gradle`
eylemi veya senin bilgisayarına kurduğun Gradle'ı kullanır. Bu yüzden `./gradlew` değil
`gradle` komutunu kullanacaksın.
...
2. GitHub'da repona git → **Actions** sekmesi → **APK Derle** iş akışı otomatik başlar
   (başlamazsa iş akışını seç → **Run workflow** butonuna bas).
...
Android Studio ile klasörü aç ("Open" → proje klasörünü seç), Gradle senkronizasyonunu
bekle, `Run` bas. Android Studio kendi Gradle'ını indirir, wrapper olmasa da sorun çıkmaz.

Komut satırından: bilgisayarında [Gradle 8.9+](https://gradle.org/install/) ve JDK 17 kurulu
olmalı, sonra proje klasöründe:
